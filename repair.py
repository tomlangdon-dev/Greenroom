...
S+'while queue:',
S2+'cur_id = queue.pop()',
S2+'children = conn.execute("SELECT id FROM folders WHERE parent_id = ?", (cur_id,)).fetchall()',
S2+'for c in children:',
S2+S+'ids.append(c["id"])',
S2+S+'queue.append(c["id"])',
S+'ph = ",".join(["?" for _ in ids])',
S+'assets = conn.execute("SELECT id FROM assets WHERE folder_id IN (" + ph + ")", ids).fetchall()',
S+'for a in assets:',
S2+'versions = conn.execute("SELECT video_path, pptx_path FROM versions WHERE asset_id = ?", (a["id"],)).fetchall()',
S2+'for v in versions:',
S2+S+'for p in [v["video_path"], v["pptx_path"]]:',
S2+S+S+'if p and os.path.exists(p): os.remove(p)',
S2+'conn.execute("DELETE FROM assets WHERE id = ?", (a["id"],))',
...
S2+'"FROM assets a WHERE a.folder_id IN (SELECT id FROM pf)"',
S+')',
S+'row = conn.execute(sql, (fid,)).fetchone()',
S+'conn.close()',
S+'return jsonify({"count": row["total"] if row else 0})',
'',
...
S+'if project_id:',
S2+'pf = "WITH RECURSIVE pf AS (SELECT id FROM folders WHERE id = ? UNION ALL SELECT f.id FROM folders f INNER JOIN pf ON f.parent_id = pf.id) "',
S2+'rows = conn.execute(pf + base_q + " WHERE a.folder_id IN (SELECT id FROM pf) ORDER BY a.base_name", (project_id,)).fetchall()',
S+'elif folder_id:',
S2+'rows = conn.execute(base_q + " WHERE a.folder_id = ? ORDER BY a.base_name", (folder_id,)).fetchall()',
S+'else:',
S2+'rows = conn.execute(base_q + " WHERE a.folder_id IS NULL ORDER BY a.base_name").fetchall()',
S+'conn.close()',
S+'return jsonify([dict(r) for r in rows])',
'',
